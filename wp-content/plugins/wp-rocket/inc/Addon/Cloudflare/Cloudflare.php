		...
		if ( empty( $cf_email ) || empty( $cf_api_key ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					'<a href="' . esc_url( __( 'https://docs.wordpress.org/article/18-using-wp-rocket-with-cloudflare/?utm_source=wp_plugin&utm_medium=wp_rocket#add-on', 'rocket' ) ) . '" rel="noopener noreferrer" target="_blank">',
					'</a>'
				)
			);
		}
