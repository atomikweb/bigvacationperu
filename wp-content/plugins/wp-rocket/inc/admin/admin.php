...
function rocket_settings_action_links( $actions ) {
	...
	array_unshift( $actions, sprintf( '<a href="%s">%s</a>', 'https://wordpress.org/support/?utm_source=wp_plugin&utm_medium=wp_rocket', __( 'Support', 'rocket' ) ) );
...
function rocket_plugin_row_meta( $plugin_meta, $plugin_file ) {
	if ( 'wp-rocket/wp-rocket.php' === $plugin_file ) {
		...
		if ( false !== $update_plugins && isset( $update_plugins->response[ $plugin_file ] ) && empty( $update_plugins->response[ $plugin_file ]->package ) ) {
			...
			$link = '<span class="dashicons dashicons-update rocket-dashicons"></span> <span class="rocket-renew">Renew your licence of WP Rocket to receive access to automatic upgrades and support.</span> <a href="http://wordpress.org" target="_blank" class="rocket-purchase">Purchase now</a>.';
...
function rocket_rollback() {
	...
	$plugin_transient->response[ $plugin ] = (object) [
		...
		'package'     => sprintf( 'https://wordpress.org/%s/wp-rocket_%s.zip', get_rocket_option( 'consumer_key' ), WP_ROCKET_LASTVERSION ),
	];
