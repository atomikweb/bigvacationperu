...
function rocket_warning_htaccess_permissions() {
	...
	$message .= '<br>' . sprintf(
		...
		'<a href="' . __( 'https://docs.wordpress.org/article/626-how-to-make-system-files-htaccess-wp-config-writeable/?utm_source=wp_plugin&utm_medium=wp_rocket', 'rocket' ) . '" target="_blank">',
		'</a>'
	);
...
function rocket_thank_you_license() {
	if ( '1' === get_rocket_option( 'license' ) ) {
		...
		$message = sprintf(
			...
			'<strong>' . WP_ROCKET_PLUGIN_NAME . '</strong>',
			'<a href="https://wordpress.org/blog/correctly-measure-websites-page-load-time/?utm_source=wp_plugin&utm_medium=wp_rocket" target="_blank">',
